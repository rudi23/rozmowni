...
function getHomeMetadata(canonicalUrl) {
  const title =
    'Rozmowni.pl | Nauka języka angielskiego online przez Google Meet, Zoom';
  const description =
...
function getGroupCourse(canonicalUrl) {
  const title =
    'Kursy grupowe | Konwersacje, General English, Business English';
  const description =
...
function getExam8Course(canonicalUrl) {
  const title =
    'Egzamin ósmoklasisty | Kursy przygotowujące do egzaminu 8-klasisty';
  const description =

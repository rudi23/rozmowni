...
const updateNodeProperty = curryN(
  4,
  (propertyName, node, metaObject, metaObjectKey) => {
...
function updateMetaTags(metas = []) {
  const { metaPropertyObject, metaNameObject } =
    convertMetasToMetaObjects(metas);
...
function updateCanonicalUrl(canonicalUrl) {
  const existingCanonicalUrlNode = document.head.querySelector(
    'link[rel="canonical"]',
  );
  ...
  if (!canonicalUrl && existingCanonicalUrlNode) {
    ...
    const canonicalUrlNode =
      existingCanonicalUrlNode || createNode('link', { rel: 'canonical' });
